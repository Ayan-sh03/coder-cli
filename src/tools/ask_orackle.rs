...
pub fn ask_orackle(query: &str) -> Result<String, String> {
    ...
    let user_message = Message {
        role: "user".to_string(),
        content: Some(format!("Main agent is stuck with this problem and needs insights:\n\n{}", query)),
        tool_calls: None,
        tool_call_id: None,
    };
    ...
    // Make the LLM call
    let response = match rt.block_on(llm.chat_once_no_stream(&messages, &tools)) {
            ...
            if let Some(tool_calls) = response.tool_calls {
                ...
                for tool_call in tool_calls {
                    match tool_call.function.name.as_str() {
                        ...
                        "search_in_files" => {
                            if let Ok(args) = serde_json::from_str::<Value>(&tool_call.function.arguments) {
                                if let (Some(pattern), Some(path)) = (args["pattern"].as_str(), args["path"].as_str()) {
                                    let case_sensitive = args.get("case_sensitive").and_then(|v| v.as_bool());
                                    match crate::tools::search_in_files(pattern, path, case_sensitive) {
                                        Ok(results) => {
                ...
                if !accumulated_insights.is_empty() {
                    ...
                    Ok("Orackle: Unable to provide insights due to tool execution failures.".to_string())
                }
            } else {
